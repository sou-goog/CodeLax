...
class SidebarProvider {
    ...
    handleMessage(msg) {
        switch (msg.command) {
            ...
            case "refresh":
                ...
                break;
            case "openBrowser":
                vscode.env.openExternal(vscode.Uri.parse(`${(0, api_1.getServerUrl)()}/dashboard/reviews`));
                break;
            case "reviewFile":
    ...
    setLoading(loading) { this._loading = loading; this.render(); }
    setError(error) { this._error = error; this._loading = false; this.render(); }
    setReviews(reviews, repoName) {
    ...
    buildHtml() {
        ...
        if (this._error)
            return this.buildErrorScreen(this._error);
